...
// Diferente de ativarFiltroClicavel (usado por Tributação/Segmento/Gerente,
// que são single-select): aqui cada clique liga/desliga aquela unidade sem
// desmarcar as outras, permitindo comparar várias de uma vez. "Todas" é um
// caso especial que limpa a seleção inteira.
...
const STATUS_ORDEM = ["Fechado", "Importado Contábil", "Simulando", "OK - Com GC", "Não Importado"];
// Só ajusta a grafia exibida pra bater com o Radar Fiscal onde o
// significado é o mesmo — não inventa correspondência pra status sem
// equivalente lá (ex.: "Importado Contábil" fica como está).
const STATUS_ROTULOS = {
  "OK - Com GC": "Com o GC",
  "Não Importado": "Não importado",
};
...
// Clicar no cabeçalho do segmento só expande/recolhe (não filtra) — pra
// filtrar por segmento, usa o botão "Fixar". Card de tributação dentro (o
// mesmo estilo de "Por Tributação") continua filtrando (el.regime) num
// clique direto, igual antes.
function renderizarFaixaSegmento(container, chavePrincipal, chaveSecundaria, filtroPrincipal, filtroSecundario) {
      ...
      return `
        <div class="quebra-faixa${aberto ? "" : " colapsado"}${ativo}" title="${nome}">
          <div class="quebra-faixa-cabecalho" data-valor="${nome.replace(/"/g, "&quot;")}">
            <button type="button" class="quebra-faixa-toggle" aria-label="Mostrar segmento" aria-expanded="${aberto}"><i class="seta"></i></button>
            <div class="quebra-nome">${nome}</div>
            <div class="quebra-total-num">${g.total.toLocaleString("pt-BR")}</div>
            <button type="button" class="quebra-faixa-fixar${ativo ? " fixado" : ""}" aria-label="${ativo ? "Remover filtro deste segmento" : "Filtrar por este segmento"}" title="${ativo ? "Remover filtro" : "Fixar filtro"}">
  ...
  function alternarAberto(cabecalho) {
    ...
    botaoToggle.setAttribute("aria-label", colapsado ? "Mostrar segmento" : "Ocultar segmento");
  }
  ...
  // atrapalhar quem só quer abrir o segmento e ver as tributações dentro.
  ...
  // Botão "Fixar": único jeito de filtrar por este segmento.
...
const QUEBRA_CONFIG = {
  ...
  segmento: { chave: "Segmento", subChave: "Tributacao", filtroEl: () => el.segmento, subFiltroEl: () => el.regime },
};
let abaQuebraAtiva = "regime";

function renderizarQuebras() {
  const cfg = QUEBRA_CONFIG[abaQuebraAtiva];
  el.quebraConteudo.classList.toggle("quebra-grid--faixas", abaQuebraAtiva === "segmento");
  if (cfg.subChave) {
    renderizarFaixaSegmento(el.quebraConteudo, cfg.chave, cfg.subChave, cfg.filtroEl(), cfg.subFiltroEl());
  } else {
...
function carregarStatus() {
  fetch("data/analise_balanco/status.json?" + Date.now())
    .then((r) => r.json())
    .then((s) => {
...
// Quantas empresas tiveram a primeira importação (DataImportacao) em cada
// dia — vem direto da planilha (cada linha já tem sua própria data), não de
// um histórico acumulado por execução do robô. Respeita os filtros ativos
// (mesmo conjunto `filtrados` dos cards/ranking).
function contarImportacoesPorDia() {
  const dias = new Map();
  filtrados.forEach((r) => {
    if (!r.DataImportacao) return;
    const dia = r.DataImportacao.slice(0, 10);
...
// resto do portal) com o total de empresas com primeira importação naquele
// dia. Barra (não linha) porque é uma contagem discreta por dia, não um
// total acumulado — a linha insinuava uma continuidade entre os pontos que
// não existe de verdade. Todo dia do histórico mostra sua data no eixo (sem
// decimar rótulo), mesmo critério do Radar Fiscal.
function renderizarEvolucao() {
  const container = el.evolucaoGrafico;
  if (!container) return;

  const historico = contarImportacoesPorDia();
  if (historico.length < 2) {
    container.innerHTML = `<p class="evolucao-vazio">Sem dias suficientes com Primeira Importação no filtro atual para montar o gráfico.</p>`;
    return;
  }
...
function carregarDados() {
  fetch("data/analise_balanco/analise_balanco_dados.json?" + Date.now())
    ...
    .then((json) => {
      ...
      popularSelect(el.gerente, new Set(dados.map((r) => r.Gerente).filter(Boolean)));
      ...
      popularSelect(el.tRegime, new Set(dados.map((r) => r.Tributacao).filter(Boolean)));
      popularSelect(el.tGerente, new Set(dados.map((r) => r.Gerente).filter(Boolean)));
    ...
    .catch(() => {
      el.corpo.innerHTML = `<tr><td colspan="8">Nenhum dado exportado ainda — rode o robô (backend/orquestrador.py).</td></tr>`;
    });
}
...
[el.tBusca, el.tUnidade, el.tSegmento, el.tRegime, el.tStatus, el.tDocumentacao, el.tGerente].forEach((campo) => {
