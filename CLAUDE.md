...
- **Exclusão de Unidade (2026-08-20)**: `static/script.js` agora remove "MG EXPRESS" de `dados` em `carregarDados()` via `UNIDADES_EXCLUIDAS = ["MG EXPRESS"]`, igual ao Radar Fiscal — a pedido do usuário, revertendo a decisão anterior de não copiar essa regra. Unidade some de todos os filtros/cards/ranking/evolução, não só do chip de Unidade.
